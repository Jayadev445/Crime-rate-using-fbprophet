...
def index():
    ...
    if request.method == 'POST':
        ...
        daily_crime['ds'] = pd.to_datetime(daily_crime['ds'])  # Ensure it's datetime format
        # Aggregate crime counts per month
        # df_crime['Month'] = df_crime['Date'].dt.to_period('M').dt.to_timestamp()
        # monthly_crime = df_crime.groupby('Month').size().reset_index(name='CrimeCount')
        ...
        if model_name == 'no_outliers_model.pkl':
